#!/usr/bin/env python3
"""
Generate synthetic RGB images from trained zoom level 23 autoencoder.

This script generates synthetic images by sampling from the latent space
...
def generate_synthetic_images(decoder, latents, device='cuda', num_samples=20, method='gaussian'):
    """Generate synthetic images from latents."""
    decoder.eval()
    ...
    # Generate images
    with torch.no_grad():
        synthetic_imgs = decoder(latent_tensor)  # (B, 3, H, W)
...
def visualize_samples(images, output_file='synthetic_samples.png', n_cols=5):
    ...
    plt.suptitle('Synthetic RGB Images (Zoom Level 23)', fontsize=14)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='Generate synthetic RGB images')
    ...
    parser.add_argument('--output_dir', type=str, default='synthetic_zoom23',
    ...
    print(f"\nGenerating {args.num_samples} synthetic images using {args.sample_method} method...")
    ...
    print(f"\nDone! Generated {args.num_samples} synthetic images")
